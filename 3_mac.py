...
APP_VERSION = "0.21 BETA"  # updated for beta release
...
class UpdateAvailableDialog(QDialog):
    ...
    def __init__(self, parent=None, latest_version="", current_version="", notes=""):
        ...
        version_label.setFont(QFont("Tahoma", 11))
        layout.addWidget(version_label)
        
        # Patch notes section
        if notes:
            notes_title = QLabel("📋 PATCH NOTES:")
            notes_title.setFont(QFont("Tahoma", 11, QFont.Bold))
...
def setup_tesseract_path():
    """Setup Tesseract path for macOS if needed."""
    ...
    # Try paths in order of preference for macOS
    possible_paths = [
        # PyInstaller bundled path (highest priority)
        os.path.join(base_path, "Tesseract-OCR", "tesseract"),
        # Local development path
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "Tesseract-OCR", "tesseract"),
    ...
    debug_print("[OCR] Please install Tesseract via: brew install tesseract")
    return False
...
def setup_tessdata_path():
    ...
    if os.path.exists(tessdata_path):
        ...
        # Try system tessdata paths for macOS
        ...
        for path in system_tessdata_paths:
            if os.path.exists(path):
                ...
                debug_print(f"[OCR] Using system tessdata at: {path}")
                return True
        
        debug_print(f"[OCR] Tessdata not found at: {tessdata_path}")
        debug_print("[OCR] Please install Tesseract language data via: brew install tesseract-lang")
        return False
    ...
    try:
        ...
        with Image.open(image_path) as img:
            ...
            if sys.platform.startswith('win'):
                ...
                # macOS/Linux: Use regular pytesseract
...
class EditableLine(QLineEdit):
    ...
    def __init__(self, text=""):
        super().__init__(text); self.setFont(QFont("Tahoma", 12)); self.setReadOnly(True); self.setFrame(False)
...
class QuestionRow(QWidget):
    ...
    def __init__(self, text, index):
        ...
        self.num = QLabel(f"{index}."); self.num.setFont(QFont("Tahoma",12,QFont.Bold)); self.num.setFixedWidth(26)
...
class App(QWidget):
    def __init__(self):
        ...
        hdr = QHBoxLayout(); title = QLabel("EFL Cafe Wizard"); title.setFont(QFont("Tahoma", 18, QFont.Bold))
        ...
        self.title_edit = QLineEdit(self.unit_title); self.title_edit.setPlaceholderText("Unit Title Goes Here..."); self.title_edit.setFont(QFont("Tahoma", 12))
        ...
        self.level_combo = QComboBox(); self.level_combo.setFont(QFont("Tahoma", 11)); self.level_combo.addItems(LEVEL_TOKENS)
        ...
        # Topics/spec
        self.topics_box = QTextEdit(); self.topics_box.setReadOnly(False); self.topics_box.setMinimumHeight(160)
        self.topics_box.setStyleSheet(get_standard_textedit_style())
        self.topics_box.setPlaceholderText(
            "Paste your spec here. Example:\n"
            "Unit 1 - Step Forward\n"
            "* talk about how we deal with change\n"
            "* talk about past difficulties\n"
            " talk about daily life in the past\n"
            " vocab: accept, adapt, be a step forward, be capable of, resist, get through, get a grip, cope with, tackle, underestimate\n\n"
        ...
        self.btn_export_docx = QPushButton("Export to DOCX"); self.btn_export_docx.setFont(QFont("Tahoma", 11)); self.btn_export_docx.clicked.connect(self.export_docx)
        self.btn_export_docx.setStyleSheet(get_standard_button_style("success"))
        self.btn_export_pdf = QPushButton("Export to PDF"); self.btn_export_pdf.setFont(QFont("Tahoma", 11)); self.btn_export_pdf.clicked.connect(self.export_pdf)
    ...
    def on_ocr_finished(self, unit_title, topics_text, vocab_list):
        ...
        # Update vocabulary if any
        if vocab_list:
            current_text = self.topics_box.toPlainText()
            if not current_text.strip().endswith('vocab:'):
                vocab_text = f"\n\nvocab: {', '.join(vocab_list)}"
                self.topics_box.setText(current_text + vocab_text)
            ...
            for url in urls:
                if url.isLocalFile():
                    ...
                    if file_ext in ['.png', '.jpg', '.jpeg', '.bmp', '.tiff', '.gif', '.pdf']:
                        # Add visual feedback by highlighting the topics box
